...
class ZLA8015DDriverNode(Node):
    ...
    def wheel_speed_callback(self, msg):
        ...
        if len(msg.data) == 2:
            ...
            '''
            # Check từng bánh bên trái
            if left_speed_front is None:
                self.get_logger().error("Lỗi đọc tốc độ: Bánh TRÁI TRƯỚC")

            if left_speed_back is None:
                self.get_logger().error("Lỗi đọc tốc độ: Bánh TRÁI SAU")

            # Check từng bánh bên phải
            if right_speed_front is None:
                self.get_logger().error("Lỗi đọc tốc độ: Bánh PHẢI TRƯỚC")

            if right_speed_back is None:
                self.get_logger().error("Lỗi đọc tốc độ: Bánh PHẢI SAU")
            '''
